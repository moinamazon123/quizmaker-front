...
audioSource = null;
  ...
  quizNotCreatedFlag : boolean = true;
...
/** this.blobAnswerSet.forEach(blobAnsOpt=>{

  this.questionObj.questionSeq = blobAnsOpt.questionSeq;

  this.questionObj.responseList.forEach(response=>{
   
    response.blobUrl = AppConst.serverPath+"/quizmaker/getImage/"+this.quizObj.quizIndex+"/"+(questnSet.length+1)+"_"+response.responseSeq+"_Answer_Image.png";
  })
...
console.log( "Before delete :",this.quizObj.questionList);
   ...
   // this.quizObj.questionList.push(questObj);
    ...
    // Delete bolb Answer Object //

    this.blobAnswerSet.forEach(x=>{
        if(x.questionSeq === i){
          x.answerBlob =[];
        }
...
console.log(this.quizObj.questionList ,i,this.blobAnswerSet);
...
this.quizObj.questionList.forEach(question =>{
 ...
 this.blobQuestionObject.forEach(fileBlob =>{
  ...
  if(fileBlob.questionSeq === i){
    ...
    reader.readAsDataURL(fileBlob.file);
    
    reader.onload = (event: any) => {
    // this.audioSource =   URL.createObjectURL(file);//event.target.result;
    this.blobQuestionUrl = this.sanitizer.bypassSecurityTrustUrl(URL.createObjectURL(fileBlob.file));
...
editQuestionSet(i){
  ...
  checkRole() {
  console.log("inside check role")
    this.userService.getRoleByUserId(this.loggedInUser).subscribe(
      res => {
...
else
{
  ...
  */
  ...
  getQuizCreator(creatorId:number){

    this.userList.forEach(user=>{

        if(user.id === creatorId ){
          this.quizCreatorName = user.username;
        }

    })
      return this.quizCreatorName;
  }
  ...
  this.quizList.forEach(quiz=>{
        ...
        if(loggedinuser == quiz.creator && quiz.activity.activity_id ===2){
  ...
  saveSetting(settingquiz:Quiz){
    this.assigneeUserIds =[];
    this.save();

    this.assigneeUserObj.name.forEach(element => {
      this.assigneeUserIds.push(element.item_id);
    });
    console.log(this.assigneeUserIds.join(','));
     console.log(this.quizSettingObj);
this.quizObj.max_attempt = this.quizSettingObj.max_attempt;
this.quizObj.invigilator = this.quizSettingObj.invigilator;
this.quizObj.reviewer = this.quizSettingObj.reviewer;
this.quizObj.show_score = this.quizSettingObj.show_score;
this.quizObj.show_answer = this.quizSettingObj.show_answer;
this.quizObj.time = this.quizSettingObj.time;
this.quizObj.scheduleDateTime = this.quizSettingObj.scheduleDateTime;
this.quizObj.assigneeUserIdList = this.assigneeUserIds.join(',');
...
this.quizObj.assigneeRoleList = this.assigneeRoleList.length>0?this.assigneeRoleList.toString():
 ...
 this.quizObj.assigneeStateList = this.assigneeStateList.length>0?this.assigneeStateList.toString():
                                  (this.quizObj.assigneeStateList.length>0?
                                    this.quizObj.assigneeStateList.toString():null);
...
this.editQuestionObj =new Question();
  ...
  deleteQuiz(){
    ...
    this.quizService.deleteQuiz(this.quizIdDel).subscribe(
      res => {
       // console.log(res.json());
  ...
  ngOnInit() {
   ...
   this.loggedInUser =  this.getWithExpiry("userId");
    ...
    if(this.loggedInUser == null){
      ...
      localStorage.setItem("lastURL","/quizCreate");
      this.router.navigate(['/login']).then(s =>location.reload());;
...
this.quizSettingObj.invigilator= null; 
  ...
  selectState(item: any){
    ...
    if(this.quizObj.assigneeStateList!=null){
      this.assigneeStateList = this.quizObj.assigneeStateList.split(",");
      if(!this.assigneeStateList.includes(item.item_text)){
        this.assigneeStateList.push(item.item_text);
      }
    } else {
    this.assigneeStateList.push(item.item_text);
    }
    this.populateCities(item);
    console.log( this.assigneeStateList);
  }
  ...
  resetRoleCity(){
    ...
    this.selectedCities=[] ;
    this.selectedStates=[];
    this.quizObj.assigneeStateList=null;
    this.quizObj.assigneeCityList=null;
    this.quizObj.assigneeRoleList=null;
...
validateRandom(){
  ...
  if(this.quizObj.questionList.length === Number(this.quizSettingObj.randomQuestions)){
    this.randomMsg = " Random Questions Cant be same as quiz question size";
    } else {
      this.randomMsg = "";
    }
...
this.quizObj.category = this.category;
this.quizObj.creator = this.getWithExpiry("userId");
this.quizObj.quizIndex = this.quizList.length+1;
...
if(this.updateFlag){
   ...
   this.quizService.saveQuiz(this.quizEditObj).subscribe(
    ...
    this.router.navigate(['/quizCreate']).then(s =>location.reload());
  }
...
uploadAnswerObject(event,responseSeq,questionSeq){

  const distinctBlobAnswer = this.blobAnswerSet.filter(
    (blobAnsw, i, arr) => arr.findIndex(t => t.questionSeq === blobAnsw.questionSeq) === i
  );
  this.blobAnswerSet = distinctBlobAnswer;

 

  this.blobAnswerFlag = true;
  let blobAnswerMap : AnswerSeqMap=new AnswerSeqMap();
  console.log(this.ansFileList);
  console.log(responseSeq);
  console.log(this.quizObj.questionList);
  const file = (event.target as HTMLInputElement).files[0];
   /** Delete Blob if already there in the response sequence */
  this.blobAnswerSeqMapList.forEach(x=>{
      if(x.responseSeq === responseSeq && x.questionSeq === this.quizObj.questionList.length+1){
        let index =   this.blobAnswerSeqMapList.indexOf(x);
        this.blobAnswerSeqMapList.splice(index, 1 );
      }
  });
  /** */
  blobAnswerMap.responseSeq = responseSeq;
  blobAnswerMap.file = file;
  blobAnswerMap.questionSeq = this.quizObj.questionList.length+1;
  ...
  this.blobAnswerSet.sort(function(a, b) {
    return a.reponseSeq - b.reponseSeq;
  });
...
uploadAnswerObjectForEdit(event,responseSeq,questionSeq){


  this.blobAnswerFlag = true;let blobAnswerSeqMapList =[];
  let blobAnswerMap : AnswerSeqMap=new AnswerSeqMap();
 
  const file = (event.target as HTMLInputElement).files[0];
   /** Delete Blob if already there in the response sequence */
  this.blobAnswerSet.forEach(x=>{
      if(x.questionSeq === questionSeq){
        x.answerBlob.forEach(y=>{
          if(y.responseSeq === (responseSeq+1)){
            let index =    x.answerBlob.indexOf(y);
            x.answerBlob.splice(index, 1 );
          }
  ...
  console.log(this.blobAnswerSet);
  blobAnswerMap.responseSeq = responseSeq+1;
  blobAnswerMap.file = file;
  blobAnswerMap.questionSeq = questionSeq;
  blobAnswerSeqMapList.push(blobAnswerMap);
 
  let blobAnswer : BlobAnswer = new BlobAnswer() ;
  blobAnswer.answerBlob = blobAnswerSeqMapList;
  //blobAnswer.reponseSeq = responseSeq;
  blobAnswer.questionSeq = questionSeq;
  this.blobAnswerSet.push(blobAnswer);
...
getAnswerBlob(responseSeq,questionSeq ){
  ...
  this.blobAnswerFlag = true;
  console.log(this.blobAnswerSet,this.ansFileList);
...
getCurrentAnswerBlob(i,questionSeq){
  this.blobAnswerFlag = true;
  // Existing Bug where arraylist size is equal to total image answers options , hence mark it distnict
  const distinctBlobAnswer = this.blobAnswerSet.filter(
    (blobAnsw, i, arr) => arr.findIndex(t => t.questionSeq === blobAnsw.questionSeq) === i
  );
  this.blobAnswerSet = distinctBlobAnswer;
  // Sort the Answers According to response seq
  this.blobAnswerSet.forEach(x=>{
            x.answerBlob.sort(function(a, b) {
          return a.responseSeq - b.responseSeq;
        });
      
  })

  console.log(this.blobAnswerSet,i,questionSeq);
  let blobUrl=null;
  this.blobAnswerSet.forEach(blonAns=>{
    blonAns.answerBlob.forEach(obj=>{
      if(obj.responseSeq === i){
        blobUrl = this.sanitizer.bypassSecurityTrustUrl(URL.createObjectURL(obj.file));
      }
...
getBlobQuestionUrl(editQuestionObject : Question,questionSeq:number){
  console.log(editQuestionObject,questionSeq,this.blobQuestionObject);
    let blobUrl = null;
...
saveColorSetting(){
  ...
  this.blobAnswerSet.forEach(blobAnsOpt=>{
    ...
    this.questionObj.responseList.forEach(response=>{
      ...
      response.blobUrl = AppConst.serverPath+"/quizmaker/getImage/"+this.quizObj.quizIndex+"/"+(this.quizObj.questionList.length)+"_"+response.responseSeq+"_Answer_Image.png";
    })
  ...
  this.quizService.saveQuizWithQuestion(this.quizObj).subscribe(
    res => {
     ...
     console.log(res.json());
      ...
      this.saveBlobAnswer(this.quizObj);
 ...
 saveBlobAnswer(quizBlob: Quiz){

  quizBlob.questionList.forEach(question => {
      if(question.question_type ==='Image(Checkbox)'){
        question.responseList.forEach(blobAnswer=>{
            this.quizService.addBlobAnswer(quizBlob,blobAnswer,question,this.blobAnswerSet).subscribe(
              res => {
               // console.log(res.json());
              console.log(res);
              },
